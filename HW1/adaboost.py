...
class Adaboost:
    ...
    def train(self, dataset):
        ...
        for t in range(self.T):
            ...
            clf, error = self.mySelectBest(featureVals, iis, labels, features, weights)
            #update weights
            accuracy = []
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        '''
        build a 0, 1 np array "haar" by checking the corresponding position in featureVals.
        check whether the value in that position is less than zero and show as integer 1 if yes otherwise 0.
        
        Then, for every single feature, we calculate the error and append it into the array called errors.
        Finally, we use argmin to find the position of the minimum value 
        and return the corresponding classifier and error value.
        '''
        haar = (featureVals < 0).astype(int)
        errors = np.empty(0)
        for arr in haar:
          sum = 0
          for  i , val in enumerate(arr):
            sum += weights[i] * abs(val - labels[i])
          errors = np.append(errors, sum)

        minpos = np.argmin(errors)
        bestClf = WeakClassifier(features[minpos])
        bestError = errors[minpos]
        # End your code (Part 2)
        return bestClf, bestError
    
    def mySelectBest(self, featureVals, iis, labels, features, weights):
        haar = (featureVals<0).astype(int)
        errors = np.empty(0)
        for arr in haar:
          sum = 0
          for i, val in enumerate(arr):
            sum += (weights[i]) * abs(val - labels[i]) * 1.1
          errors = np.append(errors, sum)
          

        minpos = np.argmin(errors)
        bestClf = WeakClassifier(features[minpos],-10)
        bestError = errors[minpos]

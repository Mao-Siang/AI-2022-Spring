...
class Agent():
    ...
    def learn(self, state, action, reward, next_state, done, count):
        ...
        # if done:
        #     self.qtable[state[0],state[1],state[2],state[3],action] = 0
        # else: 
          
        self.qtable[state[0],state[1],state[2],state[3],action] = (1-self.learning_rate)*self.qtable[state[0],state[1],state[2],state[3],action] + self.learning_rate*(reward + self.gamma*np.max(self.qtable[next_state[0],next_state[1],next_state[2],next_state[3]]))
        # End your code

        # You can add some conditions to decide when to save your table
        
        if count % 100 == 0: np.save("./Tables/cartpole_table.npy", self.qtable)
    ...
    def check_max_Q(self):
        ...
        """
        # Begin your code
        fstate = self.discretize_observation(self.env.reset())
        max_q = np.max(self.qtable[fstate[0],fstate[1],fstate[2],fstate[3]])
        return max_q 
        # End your code

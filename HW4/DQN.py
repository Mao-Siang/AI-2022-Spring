...
class Agent():
    ...
    def learn(self):
        ...
        states, actions, rewards, next_states, done = transitions
    
        batch_state = torch.tensor(states)
        batch_action = torch.tensor(actions)
        batch_reward = torch.tensor(rewards)
        batch_next_state = torch.tensor(next_states)
        batch_done = torch.tensor(done).float()

        q_value = self.evaluate_net.forward(batch_state).gather(1, batch_action.view(-1,1))
       
        max_next_q_value = self.target_net.forward(batch_next_state).detach().max(1)[0]*(1-batch_done)
        target_q_value = (batch_reward + self.gamma * max_next_q_value).unsqueeze(1)
    ...
    def choose_action(self, state):
        ...
        with torch.no_grad():
            ...
            if np.random.uniform() < self.epsilon:
                ...
                q = self.evaluate_net.forward(torch.tensor(state))
    ...
    def check_max_Q(self):
        ...
        """
        # Begin your code
        max_q = self.target_net.forward(torch.tensor(self.env.reset())).detach().max(0)[0]
